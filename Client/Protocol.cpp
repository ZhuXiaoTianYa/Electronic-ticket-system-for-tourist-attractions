...
bool deLength(std::string& package, std::string* text)
{
    ...
    *text = package.substr(pos + LINE_SEP_LEN, text_len);
    ...
    //{
    //    _status = "";
    //    logMessage(DEBUG, "Login_Class::response_deserialize input = %s", in.c_str()); // ← 加上这行
    //    Json::Value root;
    //    Json::Reader reader;

    //    if (!reader.parse(in, root)) {
    //        logMessage(ERROR, "JSON parse failed: %s", in.c_str());
    //        return false;
    //    }
    ...
    //    }
    //    else {
    //        logMessage(ERROR, "[response_deserialize] missing or invalid 'status': %s", in.c_str());
    //        return false;
    //    }
