...
bool areas_request(std::string &aid, int sock)
{
	...
	while (true)
	{
		...
		if (ParseOnePackage(package, &response))
		{
			...
			{
				areas_name = areas_._name;
				areas_introduction = areas_._introduction;
				areas_remaining_tickets = areas_._remaining_tickets;
				areas_tickets_total = areas_._tickets_total;
				return true;
			}
			else
			{
